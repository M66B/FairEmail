...
public interface DaoMessage {
    ...
    @Query("SELECT message.*" +
            ...
            ", '[' || (SELECT group_concat(`from`, ',') FROM (SELECT DISTINCT `from` FROM message m0 WHERE CASE WHEN :threading THEN m0.thread = message.thread ELSE m0.id = message.id END ORDER BY m0.received DESC LIMIT 3)) || ']' AS senders" +
            ", '[' || (SELECT group_concat(`to`, ',') FROM (SELECT DISTINCT `to` FROM message m0 WHERE CASE WHEN :threading THEN m0.thread = message.thread ELSE m0.id = message.id END ORDER BY m0.received DESC LIMIT 3)) || ']' AS recipients" +
